import { Decoder, UNIT_FEET, UNIT_METERS } from "./decoder.mjs";
import {
    msgLen,
    LONG_MSG_BITS as long_msg_bits,
    SHORT_MSG_BITS as short_msg_bits,
} from "./mode-s-msglen.mjs";
...
// any resolution.
for (let i = 0; i <= 128; i++) {
    for (let q = 0; q <= 128; q++) MAG_LUT[i * 129 + q] = Math.round(Math.sqrt(i * i + q * q) * 360);
}
...
Demodulator.prototype.detectMessage = function(mag, maglen, onMsg) {
    ...
    for (let j = 0; j < maglen - FULL_LEN * 2; j++) {
        ...
        if (useCorrection) {
            ...
            if (j && detectOutOfPhase(mag, j)) applyPhaseCorrection(mag, j);
        } else {
        ...
        for (i = 0; i < long_msg_bits * 2; i += 2) {
            ...
            else bits[i / 2] = 0; // (low < high) for exclusion
        }

        // Restore the original message if we used magnitude correction.
        if (useCorrection) memcpy(mag, j + PREAMBLE_US * 2, aux, 0, aux.length);
...
function applyPhaseCorrection(mag, offset) {
    ...
    for (let j = 16; j < (long_msg_bits - 1) * 2; j += 2) {
        if (mag[offset + j] > mag[offset + j + 1]) mag[offset + j + 2] = (mag[offset + j + 2] * 5) / 4; // One

        else mag[offset + j + 2] = (mag[offset + j + 2] * 4) / 5; // Zero
    }
}

function memcpy(dst, dstOffset, src, srcOffset, length) {
    for (let i = srcOffset; i < length; i++) dst[dstOffset + i] = src[i];
}
